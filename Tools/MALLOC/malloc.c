...
//static   uint8_t __attribute__((aligned(64))) mem2base[MEM2_MAX_SIZE]@(0X68000000);     /* 外扩SRAM内存池 */

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                   /* 内部SRAM内存池MAP */
//static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE]@(0X68000000 + MEM2_MAX_SIZE);      /* 外扩SRAM内存池MAP */

#elif defined(__CC_ARM)&&(!(__ARMCC_VERSION >= 6010050)) /*!< KEIL AC5编译器*/
